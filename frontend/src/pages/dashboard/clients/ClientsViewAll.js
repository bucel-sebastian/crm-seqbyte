...
function ClientsViewAll() {
  ...
  const getTableItems = async (page, rowsPerPage) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/api/v1/clients/list?page=${
        page + 1
  ...
  const handleDeleteItem = async (e) => {
    e.preventDefault();
    if (window.confirm("Sigur doresti sa stergi aceast client?")) {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/clients/delete`,
        {
          method: "POST",
          headers: {
            "x-api-nonce": nonce,
      ...
      if (response.ok) {
        ...
        if (body.status === "success") {
          toast.success("Clientul a fost sters cu success");
          setPage(0);
                      ...
                      <span className="dashboard-table-actions-container">
                        <Tooltip title="Vizualizeaza">
                          <Link
                            ...
                            to={`/dashboard/clients/preview/${item.id}`}
